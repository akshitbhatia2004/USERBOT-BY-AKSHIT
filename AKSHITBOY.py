...
lnbot = TelegramClient("akshitboy", API_ID, API_HASH).start(bot_token=token)


class AkshitBoy(TelegramClient):
...
if Config.AKSHIT_STRING:
    session_name = str(Config.AKSHIT_STRING)
    sweetie = AkshitBoy(session_name, Config.APP_ID, Config.API_HASH)
else:
