...
from userbot import CMD_LIST, LOAD_PLUG, LOGS, SUDO_LIST, Akshit, AkshitBot, bot
...
def load_module(shortname):
    if shortname.startswith("__"):
        ...
        LOGS.info("彡aͥksͣhͫᎥτ彡 ~ " + shortname)
    else:
        import userbot.utils
        ...
        mod.bot = Akshit
        mod.borg = bot
        # mod.AKSHITBOT = Akshit
        mod.AkshitBot = AkshitBot
        mod.tbot = AkshitBot
        mod.Akshit = Akshit
        mod.tgbot = bot.tgbot
        mod.Var = Var
        ...
        mod.delete_AKSHIT = delete_AKSHIT
        mod.eod = delete_AKSHIT
        mod.admin_cmd = admin_cmd
        mod.akshit_cmd = admin_cmd
        mod.sudo_cmd = sudo_cmd
        # support for AKSHITBOT originals
        sys.modules["AKSHITBOT.utils"] = userbot.utils
        sys.modules["AKSHITBOT"] = userbot
...
def load_addons(shortname):
    if shortname.startswith("__"):
        ...
        name = "userbot.plugins.AkshitBot-Addons.{}".format(shortname)
        spec = importlib.util.spec_from_file_location(name, path)
        mod = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(mod)
        ...
        spec.loader.exec_module(mod)
        # for imports
        sys.modules["AkshitBot-Addons." + shortname] = mod
...
def load_abuse(shortname):
    if shortname.startswith("__"):
        ...
        mod = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(mod)
        LOGS.info("AkshitBot-Abuse ~ " + shortname)
    else:
        import importlib
        import sys
        from pathlib import Path
    ...
    if pattern is not None:
        if pattern.startswith(r"\#"):
            ...
            if len(Config.COMMAND_HAND_LER) == 2:
                AKSHITreg = "^" + Config.COMMAND_HAND_LER
                reg = Config.COMMAND_HAND_LER[1]
            elif len(Config.COMMAND_HAND_LER) == 1:
                AKSHITreg = "^\\" + Config.COMMAND_HAND_LER
                reg = Config.COMMAND_HAND_LER
            args["pattern"] = re.compile(AKSHITreg + pattern)
    ...
    if pattern is not None:
        if pattern.startswith(r"\#"):
            ...
            if len(Config.SUDO_COMMAND_HAND_LER) == 2:
                AKSHITreg = "^" + Config.SUDO_COMMAND_HAND_LER
...
async def delete_AKSHIT(event, text, time=None, parse_mode=None, link_preview=None):
    parse_mode = parse_mode or "md"
    link_preview = link_preview or False
    time = time or 5
    if event.sender_id in Config.SUDO_USERS:
        ...
        AKSHITevent = await event.edit(
            text, link_preview=link_preview, parse_mode=parse_mode
        )
    await asyncio.sleep(time)
    return await AKSHITevent.delete()
...
def errors_handler(func):
    async def wrapper(errors):
        try:
            ...
            link = "[here](https://t.me/The_AkshitBoy)"
