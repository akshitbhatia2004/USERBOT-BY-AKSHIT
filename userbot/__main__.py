...
from userbot import LOGS, AKSHITversion, bot
from userbot.Config import Config
from userbot.helpers.runner import reload_AKSHITBOT
from userbot.start import abuses, addons, assistants, hekp, install, module, spams

l1 = Config.COMMAND_HAND_LER
l2 = Config.SUDO_COMMAND_HAND_LER
AKSHIT_PIC = "https://telegra.ph/file/f6f6f8006a1861383c566.jpg"
...
if len(sys.argv) not in (1, 3, 4):
    ...
    try:
        if Config.BOT_USERNAME is not None:
            ...
            print("♥️ Starting AkshitBot ♥️")
            bot.loop.run_until_complete(add_bot(Config.BOT_USERNAME))
            LOGS.info("🥇🔥 AkshitBot Startup Completed 🔥🥇")
        else:
            bot.start()
    except Exception as e:
...
async def killer():
    ...
    legd_mention = f"[{AKSHIT_USER}](tg://user?id={The_AkshitBoy})"
...
async def help(event):
    ...
    if event.query.user_id is not bot.uid:
        ...
        await tgbot.send_message(
            ...
            buttons=[
                [
                    Button.url(" Support ", "https://t.me/AKSHIT_USERBOT"),
                    Button.url(" Updates ", "https://t.me/OFFICIAL_USERBOTBYAKSHIT"),
                ],
                [
...
async def rel(event):
    if event.query.user_id == bot.uid:
        await event.answer(
            "Reloading 彡aͥksͣhͫᎥτ彡... Wait for few seconds...", cache_time=0, alert=True
        )
        await reload_AKSHITBOT()
    else:
        await event.answer(
...
async def users(event):
    if event.query.user_id == bot.uid:
        ...
        await tgbot.send_message(
            event.chat_id,
            message=".set var <varname> <value> ex:- .set var ALIVE_NAME AkshitBoy \n\n To Know All Var Go Back And Click On All Var",
            buttons=[
                [custom.Button.inline("Back", data="osg")],
            ],
        )
...
bot.loop.run_until_complete(akshits())
