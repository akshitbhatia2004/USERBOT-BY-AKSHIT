...
from userbot import AKSHITversion, bot
...
async def hekp():
    try:
        os.environ[
            "AKSHIT_STRING"
        ] = "String Is A Sensitive Data \nSo Its Protected By AkshitBot"
        if Config.LOGGER_ID != 0:
            await bot.send_file(
                Config.LOGGER_ID,
                AKSHIT_PIC,
                caption=f"Deployed 彡aͥksͣhͫᎥτ彡 Successfully\n\n彡aͥksͣhͫᎥτ彡 ~ {AKSHITversion}\n\nType `{l1}help` or `{l1}ping` to check!\nFor Assistant Type `.on` \n\nJoin [AkshitBot Channel](t.me/OFFICIAL_USERBOTBYAKSHIT) for Updates & [AkshitBot Chat](t.me/AKSHIT_USERBOT) for any query regarding AkshitBot",
            )
    except Exception as e:
        print(str(e))

    try:
        await bot(JoinChannelRequest("@OFFICIAL_USERBOTBYAKSHIT"))
    except BaseException:
        pass

    try:
        await bot(JoinChannelRequest("@AKSHIT_USERBOT"))
    except BaseException:
        pass
...
async def install():
    if plc == "ON":
        try:
            await bot(JoinChannelRequest("@AKSHIT_USERBOTPlugin"))
        except BaseException:
            pass
        i = 0
        chat = -1001518412326
