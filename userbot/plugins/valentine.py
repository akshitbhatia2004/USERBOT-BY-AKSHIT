...
from AKSHITBOT import ALIVE_NAME, CmdHelp
...
AKSHIT = AKSHITBOT.uid

AKSHIT = f"[{DEFAULTUSER}](tg://user?id={AKSHIT})"


@AKSHITBOT.on(admin_cmd(pattern="hvd$"))
@AKSHITBOT.on(sudo_cmd(pattern="hvd$", allow_sudo=True))
...
@AKSHITBOT.on(admin_cmd(pattern="vday$"))
@AKSHITBOT.on(sudo_cmd(pattern="vday$", allow_sudo=True))
async def gif(event):
    if event.fwd_from:
        return
