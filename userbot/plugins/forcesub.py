...
async def fs(event):
    ...
    if not await is_admin(event, AKSHITbooy):
    ...
    if args in ("off", "no", "disable"):
        ...
        rip = await check_him(channel, AKSHITbooy)
        if rip is False:
            return await event.reply(
    ...
    if chat_db:
        try:
            ...
            if rip is False:
                ...
                AKSHITboy = Config.BOT_USERNAME
                await bot(
                    EditBannedRequest(event.chat_id, event.sender_id, MUTE_RIGHTS)
                )
                response = await bot.inline_query(AKSHITboy, "pm_warn")
...
@bot.on(events.InlineQuery(pattern="AKSHITbooy"))
async def NOOBBOY(event):
    global rk
    event.builder
    AKSHITproboy = [[Button.inline("Unmute Me 😊", data="AKSHIT")]]
    NOOBBOYOP = AKSHITboy.article(
        title="FORCE SUBSCRIBE", text="fsub", buttons=AKSHITproboy
    )
...
@bot.on(events.callbackquery.CallbackQuery(re.compile(b"AKSHIT")))
