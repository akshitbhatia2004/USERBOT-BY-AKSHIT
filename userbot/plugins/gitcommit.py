...
async def download(event):
    ...
    if Var.GIT_REPO_NAME is None:
        await edit_or_reply(event, "`Please ADD Proper Github Repo Name of AKSHITBOT`")
        return
    AKSHITBOT = await edit_or_reply(event, "Processing ...")
    ...
    try:
        ...
        await AKSHITBOT.edit("Committing to Github....")
        await git_commit(downloaded_file_name, AKSHITBOT)


async def git_commit(file_name, AKSHITBOT):
    content_list = []
    ...
    for i in content_list:
        ...
        if i == 'ContentFile(path="' + file_name + '")':
            return await AKSHITBOT.edit("`File Already Exists`")
            create_file = False
    ...
    if create_file == True:
        ...
        try:
            ...
            await AKSHITBOT.edit("Cannot Upload Plugin")
    else:
        return await AKSHITBOT.edit("`Committed Suicide`")
