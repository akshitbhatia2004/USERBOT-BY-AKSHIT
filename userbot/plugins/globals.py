...
async def _(AKSHITevent):
    i = 0
    await AKSHITevent.get_sender()
    me = await AKSHITevent.client.get_me()
    AKSHIT = await eor(AKSHITevent, "`Promoting globally...`")
    my_mention = "[{}](tg://user?id={})".format(me.first_name, me.id)
    f"@{me.username}" if me.username else my_mention
    await AKSHITevent.get_chat()
    if AKSHITevent.is_private:
        user = AKSHITevent.chat
        rank = AKSHITevent.pattern_match.group(1)
    else:
        AKSHITevent.chat.title
    try:
        user, rank = await get_full_user(AKSHITevent)
    except:
        pass
    if me == user:
        await AKSHIT.edit("You can't promote yourself...")
        return
    try:
        if not rank:
            rank = "ℓεɠεɳ∂"
    except:
        return await AKSHIT.edit("**ERROR !!**")
    if user:
        telchanel = [
            d.entity.id
            for d in await AKSHITevent.client.get_dialogs()
        ...
        for x in telchanel:
            try:
                await AKSHITevent.client(EditAdminRequest(x, user, rgt, rank))
                i += 1
                await AKSHIT.edit(f"**Promoting User in :**  `{i}` Chats...")
            except:
                pass
    else:
        await AKSHIT.edit(f"**Reply to a user !!**")
    await AKSHIT.edit(
...
async def _(AKSHITevent):
    ...
    await AKSHITevent.get_sender()
    me = await AKSHITevent.client.get_me()
    AKSHIT = await eor(AKSHITevent, "`Demoting Globally...`")
    my_mention = "[{}](tg://user?id={})".format(me.first_name, me.id)
    f"@{me.username}" if me.username else my_mention
    await AKSHITevent.get_chat()
    if AKSHITevent.is_private:
    ...
    try:
        ...
        pass
    if me == user:
        await AKSHIT.edit("You can't Demote yourself !!")
        return
    try:
        if not rank:
            rank = "AKSHIT"
    ...
    if user:
        ...
        for x in telchanel:
            try:
                await AKSHITevent.client(EditAdminRequest(x, user, rgt, rank))
                i += 1
                await AKSHIT.edit(f"**Demoting Globally In Chats :** `{i}`")
            except:
                pass
    else:
        await AKSHIT.edit(f"**Reply to a user !!**")
...
async def _(event):
    AKSHIT = await eor(event, "`Gbanning...`")
    reason = ""
    ...
    if event.reply_to_msg_id:
        ...
        return await eod(
            AKSHIT, "**To gban a user i need a userid or reply to his/her message!!**"
        )
    name = (await event.client.get_entity(userid)).first_name
    chats = 0
    if userid == The_AkshitBoy:
        return await eod(AKSHIT, "🥴 **Nashe me hai kya lawde ‽**")
    ...
    if a is not None:
        ...
        gbpic = gban_pic
    gmsg = f"🥴 [{name}](tg://user?id={userid}) **Gbanned** By {AKSHIT_mention} \n\n📍 Added to Gban Watch!!\n**🔰 Total Chats :**  `{chats}`"
    if reason != "":
        gmsg += f"\n**🔰 Reason :**  `{reason}`"
    ogmsg = f"[{name}](tg://user?id={userid}) **Is now GBanned by** {AKSHIT_mention} **in**  `{chats}`  **Chats!! 😏**\n\n**📍 Also Added to Gban Watch!!**"
    if reason != "":
        ogmsg += f"\n**🔰 Reason :**  `{reason}`"
    if Config.ABUSE == "ON":
...
async def _(event):
    ...
    if event.reply_to_msg_id:
        ...
        return await eor(AKSHIT, "`Reply to a user or give their userid... `")
    name = (await event.client.get_entity(userid)).first_name
    chats = 0
    if not is_gbanned(userid):
        return await eor(AKSHIT, "`User is not gbanned.`")
    async for gfuck in event.client.iter_dialogs():
        if gfuck.is_group or gfuck.is_channel:
            try:
                await event.client.edit_permissions(
...
async def already(event):
    ...
    if len(gbanned_users) > 0:
        for user in gbanned_users:
            ...
            AKSHIT = int(hel)
            try:
                tity = await event.client.get_entity(AKSHIT)
                name = tity.first_name
            except ValueError:
                name = "User"
            GBANNED_LIST += f"📍 [{name}](tg://user?id={AKSHIT}) (`{AKSHIT}`)\n"
    else:
...
async def gkick(event):
    AKSHIT = await eor(event, "`Kicking globally...`")
    ...
    if event.reply_to_msg_id:
        ...
        return await eor(AKSHIT, "`Reply to some msg or add their id.`")
    name = (await event.client.get_entity(userid)).first_name
    chats = 0
    if userid == The_AkshitBoy:
        return await eod(AKSHIT, "**🥴 Nashe me hai kya lawde!!**")
    if str(userid) in DEVLIST:
        return await eor(AKSHIT, "**😪 I'm not going to gkick my developer!!**")
