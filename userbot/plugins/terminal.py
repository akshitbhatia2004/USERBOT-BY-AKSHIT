...
AKSHIT = str(ALIVE_NAME) if ALIVE_NAME else "Du"
...
async def dc(event):
    await event.edit(f"{AKSHIT}: Running Terminal.....")
    ...
    if cmd in ("userbot.session", "env", "printenv"):
        return await event.edit(f"{AKSHIT}: Privacy Error, This command not permitted")
    ...
    await event.client.send_file(
        ...
        "term.txt",
        reply_to=event.id,
        caption=f"{AKSHIT}: Output too large, sending as file",
    )
    os.remove("term.txt")
    return
