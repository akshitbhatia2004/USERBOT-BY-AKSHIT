...
async def nope(AKSHIT):
    AKSHIT = AKSHIT.pattern_match.group(1)
    if not AKSHIT:
        if AKSHIT.is_reply:
            (await AKSHIT.get_reply_message()).message
        else:
            await edit_or_reply(AKSHIT, "I need some text to make a tweet🚶")
            return
    tweeter = await bot.inline_query("TwitterStatusBot", f"{(deEmojify(AKSHIT))}")
    await tweeter[0].click(
        AKSHIT.chat_id,
        reply_to=AKSHIT.reply_to_msg_id,
        silent=True if AKSHIT.is_reply else False,
        hide_via=True,
    )
    await AKSHIT.delete()
