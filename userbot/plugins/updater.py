...
BOT_IS_UP_TO_DATE = "**The AKSHITBOT** is up-to-date sir."
NEW_BOT_UP_DATE_FOUND = (
    "new update found for {branch_name}\n"
    "changelog: \n\n{changelog}\n"
    "updating your AKSHITBOT ..."
)
NEW_UP_DATE_FOUND = (
    "New update found for {branch_name}\n" "`updating your AKSHITBOT...`"
)
...
AKSHITBOT_info = "https://raw.githubusercontent.com/akshitbhatia2004/USERBOT-BY-AKSHIT/AKSHITBOT/AKSHITboy-info.json"
...
async def AKSHIT_info(AKSHITBOT_info):
    infos = requests.get(AKSHITBOT_info).json()
    _version = infos["AKSHITBOT-INFO"]["version"]
    _release = infos["AKSHITBOT-INFO"]["release-date"]
    _branch = infos["AKSHITBOT-INFO"]["branch"]
    _author = infos["AKSHITBOT-INFO"]["author"]
    _auturl = infos["AKSHITBOT-INFO"]["author-url"]
...
async def upstream(event):
    ...
    if changelog == "" and not force_update:
        await event.edit(
            "\n**😎 彡aͥksͣhͫᎥτ彡 is UP-TO-DATE.**"
            f"\n\n**Version :**  {AKSHITversion}"
            f"\n**Owner :**  {AKSHIT_mention}"
    ...
    if conf == "" and not force_update:
        ...
        return await event.respond(
            f"🌚 Do `.update build` to update your **AKSHITẞø†** !!"
        )
...
async def upstream(event):
    ...
    off_repo = "https://github.com/akshitbhatia2004/USERBOTBYAKSHIT"
    os.chdir("/app")
    git_AKSHIT = f"rm -rf .git"
    try:
        await runcmd(git_AKSHIT)
    except BaseException:
        pass
    txt = "😕 `Updater cannot continue due to some problems occured`\n\n**LOGTRACE:**\n"
    try:
    ...
    ups_rem = repo.remote("upstream")
    ups_rem.fetch(ac_br)
    _version, _release, _branch, _author, _auturl = await AKSHIT_info(AKSHITBOT_info)
    await event.edit(
        f"<b><i>彡aͥksͣhͫᎥτ彡 Docker Build In Progress !!</b></i> \n\n<b><i><u>Update Information :</b></i></u> \n<b>• Branch :</b> {_branch} \n<b>• Release Date :</b> {_release} \n<b>• Version :</b> {_version} \n<b>• Author :</b> <a href='{_auturl}'>{_author}</a>",
        link_preview=False,
        parse_mode="HTML",
    )
...
async def deploy(event, repo, ups_rem, ac_br, txt):
    if HEROKU_API_KEY is not None:
        ...
        await event.edit(
            f"**Your 彡aͥksͣhͫᎥτ彡 Is UpToDate**\n\n**Version :**  __{AKSHITversion}__\n**Oɯɳҽɾ :**  {AKSHIT_mention}"
        )
    else:
        await event.edit(
