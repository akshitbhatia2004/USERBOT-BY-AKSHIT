...
async def bigspam(AKSHIT):
    if not AKSHIT.text[0].isalpha() and AKSHIT.text[0] not in ("/", "#", "@", "!"):
        AKSHIT_msg = AKSHIT.text
        AKSHITBOT_count = int(AKSHIT_msg[9:13])
        AKSHIT_spam = str(AKSHIT.text[13:])
        for i in range(1, AKSHITBOT_count):
            await AKSHIT.respond(AKSHIT_spam)
        await AKSHIT.delete()
        if LOGGER:
            await AKSHIT.client.send_message(
    ...
    if lg_id:
        if event.is_private:
            ...
            await event.client.send_message(
                lg_id,
                "#WSPAM\n"
                + f"Word Spam was executed successfully in {AKSHIT_mention} chat with : `{message}`",
            )
...
CmdHelp("spam").add_command(
    ...
    "bigspam",
    "<count> <text>",
    "Sends the text 'X' number of times. This what AKSHITBOT iz known for. The Best BigSpam Ever",
    "bigspam 5000 hello",
).add_command(
    "cspam",
    "<sentence>",
    "Spam the chat with every letter in given sentence as new message",
    "cspam AKSHITBOT IS OP",
).add_command(
    "wspam",
    "<sentence>",
    "Spams the chat with every word in given sentence as new message",
    "wspam AKSHITBOT IS OP",
).add()
