...
from AKSHITBOT.utils import admin_cmd, eor, sudo_cmd
from userbot import HEROKU_APP, AKSHITversion, bot
...
async def _(event):
    ...
    await event.edit("Rєϐοοτιиg **[ ███ ]** ...\nωαιτ ƒєω мιиυτєѕ☢️")
    await event.edit(
        f"Rebooted AKSHITBOT - {AKSHITversion} **[ ⚡ ]** ...\n**Type `.ping` or `.AKSHIT` after 5min to check if I am working**"
    )
...
async def _(event):
    if event.fwd_from:
        return
    await event.edit(
        "**[ ! ]** `⚰️Turning off bot now ... Manually turn me on later or follow step of update in @AKSHIT_Userbot` ಠ_ಠ"
    )
...
async def rel(event):
    await eor(event, "Reloading 彡aͥksͣhͫᎥτ彡... Wait for few seconds...")
