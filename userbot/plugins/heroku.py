...
USERID = bot.uid
DEFAULTUSER = ALIVE_NAME or "aͥksͣhͫꭵτϐοy"
...
async def variable(var):
    ...
    if exe == "get":
        ...
        try:
            ...
            if "AKSHIT_STRING" in variable:
                await eor(
                    var, "AKSHIT String is a Sensetive Data.\nProtected By AKSHITBOT"
                )
                return
            elif variable in heroku_var:
                await eor(var, AKSHIT)
            else:
...
async def _(event):
    ...
    AKSHIT_data = app.get_log()
    await eor(event, AKSHIT_data)
