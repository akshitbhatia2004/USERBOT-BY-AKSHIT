...
if Config.TAG_LOGGER:
    ...
    async def all_messages_catcher(event):
        ...
        AKSHIT = await event.client.get_entity(event.sender_id)
        if AKSHIT.bot or AKSHIT.verified or AKSHIT.support:
            return

        AKSHITm = f"[{get_display_name(AKSHIT)}](tg://user?id={AKSHIT.id})"
