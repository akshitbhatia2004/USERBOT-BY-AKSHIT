...
async def _(event):
    ...
    AKSHITuser = await event.client.get_me()
    if AKSHITuser.username:
        curruser = AKSHITuser.username
    else:
        curruser = "@Official_AKSHITBOT"
    uid = os.geteuid()
    if uid == 0:
...
async def _(event):
    if EVAL == "ON":
        ...
        AKSHITevent = await eor(event, "`Running ...`")
...
CmdHelp("evaluators").add_command(
    ...
    "exec",
    "<command>",
    "Execute a Terminal command on AKSHITBOT server and shows details",
).add_command(
