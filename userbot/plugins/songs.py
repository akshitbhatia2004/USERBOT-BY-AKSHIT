...
async def nope(aura):
    AKSHIT = aura.pattern_match.group(1)
    if not AKSHIT:
        if aura.is_reply:
    ...
    troll = await bot.inline_query("iLyricsBot", f"{(deEmojify(AKSHIT))}")
...
async def _(event):
    ...
    thumb_name = f"thumb{The_AkshitBoy}.jpg"
    ...
    async with borg.conversation("@DeezLoadBot") as conv:
        try:
            ...
            await borg.send_file(
                event.chat_id,
                songh,
                caption="🔆**Here's the requested song!**🔆\n`Check out` [AKSHITBOT](https://t.me/AKSHITSupport)",
            )
            await event.delete()
        except YouBlockedUserError:
