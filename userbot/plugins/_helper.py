...
msg = f"""
**⚜ Akshit𝚊𝚛𝚢 𝙰𝚏 AkshitBot ⚜**

  •        [♥️ 𝚁𝚎𝚙𝚘 ♥️](https://github.com/akshitbhatia2004/USERBOTBYAKSHIT)
  •        [♦️ Deploy ♦️](https://dashboard.heroku.com/new?button-url=https%3A%2F%2Fgithub.com%2FAKSHIT-OS%2FAKSHITBOT&template=https%3A%2F%2Fgithub.com%2FAKSHIT-OS%2FAKSHITBOT)
...
async def repo(event):
    try:
        AKSHIT = await bot.inline_query(botname, "repo")
...
async def _(event):
    ...
    if tgbotusername is not None:
        try:
            results = await event.client.inline_query(tgbotusername, "AKSHITBOT_help")
            ...
            async with bot.conversation(chat) as conv:
                try:
                    ...
                    return await AKSHIT.edit("Unblock @Botfather first.")
                await AKSHIT.edit(
...
async def yardim(event):
    ...
    if tgbotusername is not None or AKSHIT_input == "text":
        results = await event.client.inline_query(tgbotusername, "AKSHITBOT_help")
        await results[0].click(
            event.chat_id, reply_to=event.reply_to_msg_id, hide_via=True
        )
...
async def _(event):
    ...
    await event.edit("тєℓєтнση  вαѕє∂ υѕєявσт ρσωєяє∂ ву **彡aͥksͣhͫᎥτ彡** вσт")
