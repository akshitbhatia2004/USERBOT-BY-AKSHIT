...
async def spam(e):
    if ABUSE == "ON":
        ...
        Akshit = ("".join(e.text.split(maxsplit=1)[1:])).split(" ", 1)
        await e.get_reply_message()
        if len(Akshit) == 2:
            message = str(Akshit[1])
            print(message)
            a = await e.client.get_entity(message)
            g = a.id
            c = a.first_name
            username = f"[{c}](tg://user?id={g})"
            counter = int(Akshit[0])
...
async def _(e):
    ...
    if ABUSE == "ON":
        ...
        if len(e.text) > 11:
            ...
            appendable = [g]
            qeue.append(appendable)
            text = f"**Activated Reply Raid By {Akshit_mention}**"
            await e.reply(text, parse_mode=None, link_preview=None)
        else:
            await e.reply(usage, parse_mode=None, link_preview=None)
