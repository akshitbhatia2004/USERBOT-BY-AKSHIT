...
async def nope(lege):
    AKSHIT = lege.pattern_match.group(1)
    if not AKSHIT:
        if lege.is_reply:
    ...
    troll = await bot.inline_query("AniFluidbot", f".anime {(deEmojify(AKSHIT))}")
...
async def nope(lege_):
    AKSHIT = lege_.pattern_match.group(1)
    if not AKSHIT:
        if lege_.is_reply:
            (await lege_.get_reply_message()).message
        else:
    ...
    troll = await bot.inline_query("AniFluidbot", f".manga {(deEmojify(AKSHIT))}")
...
async def nope(lege_):
    ...
    troll = await bot.inline_query("AniFluidbot", f".character {(deEmojify(AKSHIT))}")
