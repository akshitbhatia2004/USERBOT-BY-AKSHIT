...
from AKSHITBOT.utils import admin_cmd, edit_or_reply, eor, sudo_cmd
...
async def get_users(event):
    ...
    AKSHIT_chat = legen_.lower
    restricted = ["@AKSHIT_Userbot", "@Official_AKSHITBOT"]
    AKSHIT = await eor(event, f"**Inviting members from** {legen_}")
    if AKSHIT_chat in restricted:
    ...
    if not sender.id == me.id:
        AKSHIT = await edit_or_reply(event, "`processing...`")
    else:
        AKSHIT = await edit_or_reply(event, "`processing...`")
    AKSHIT = await get_chatinfo(event)
    chat = await event.get_chat()
    if event.is_private:
        return await AKSHIT.edit("`Sorry, Cant add users here`")
    s = 0
    f = 0
    error = "None"

    await AKSHIT.edit(
        "**⚜️[Ͳєямιиαℓ Տτατυѕ](https://t.me/AKSHIT_Userbot)**\n\n`🔸Inviting Users.......`"
    )
    async for user in event.client.iter_participants(AKSHIT.full_chat.id):
        try:
            ...
            error = str(e)
            f = f + 1
    return await AKSHIT.edit(
        f"[τєямנиαℓ ƒιиιѕнє∂](https://t.me/AKSHIT_Userbot) \n\n🔸 Sυϲϲєѕѕƒυℓℓγ ιиνιτє∂ `{s}` ρєορℓє \n⚠️ ƒαιℓє∂ το ιиνιτє `{f}` ρєορℓє"
    )
...
async def get_users(event):
    ...
    await AKSHIT.edit("**TerminalStatus**\n\n`Collecting Users.......`")
