...
<b>⚡ <a href='https://t.me/AKSHIT_Userbot'>From DataBase of AKSHITBOT</a> ⚡ </b>
""".format(
        user_id,
        user_id,
        first_name,
...
async def info(event):
    if event.fwd_from:
        return
    AKSHIT = await eor(event, "`Analysing the chat...`")
    chat = await get_chatinfo(event)
    caption = await fetch_info(chat, event)
    try:
        await AKSHIT.edit(caption, parse_mode="html")
    except Exception as e:
        print("Exception:", e)
        await eod(AKSHIT, "`An unexpected error has occurred.`")
    return
