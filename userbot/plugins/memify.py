...
async def _(event):
    ...
    if not (_reply and (_reply.media)):
        ...
        return
    AKSHIT = await _reply.download_media()
    if AKSHIT.endswith((".tgs")):
        legen_ = await eor(event, "**Memifying 🌚🌝**")
        cmd = ["lottie_convert.py", AKSHIT, "pic.png"]
        file = "pic.png"
        ...
        stderr.decode().strip()
        stdout.decode().strip()
    elif AKSHIT.endswith((".webp", ".png")):
        legen_ = await eor(event, "**Memifying 🌚🌝**")
        pic = Image.open(AKSHIT)
        pic.save("pic.png", format="PNG", optimize=True)
        file = "pic.png"
    else:
        await eor(event, "**Memifying 🌚🌝**")
        img = cv2.VideoCapture(AKSHIT)
        tal, semx = img.read()
...
async def nope(event):
    AKSHIT = event.pattern_match.group(1)
    if not AKSHIT:
        if event.is_reply:
    ...
    troll = await bot.inline_query("DogeStickerBot", f"{(deEmojify(AKSHIT))}")
    if troll:
        await event.delete()
...
async def nope(kraken):
    AKSHIT = kraken.pattern_match.group(1)
    if not AKSHIT:
        if kraken.is_reply:
    ...
    troll = await bot.inline_query("GooglaxBot", f"{(deEmojify(AKSHIT))}")
    if troll:
        await kraken.delete()
        legen_ = await troll[0].click(Config.LOGGER_ID)
        if legen_:
...
async def nope(kraken):
    ...
    troll = await bot.inline_query("honka_says_bot", f"{(deEmojify(AKSHIT))}.")
    if troll:
        await kraken.delete()
        legen_ = await troll[0].click(Config.LOGGER_ID)
        if legen_:
