...
from AKSHITBOT.utils import admin_cmd, edit_or_reply, progress, sudo_cmd
...
async def _(AKSHIT):
    if AKSHIT.fwd_from:
        return
    reply_to_id = AKSHIT.message.id
    if AKSHIT.reply_to_msg_id:
        reply_to_id = AKSHIT.reply_to_msg_id
    event = await edit_or_reply(AKSHIT, "Converting.....")
    ...
    if event.reply_to_msg_id:
        ...
        downloaded_file_name = await AKSHIT.client.download_media(
            reply_message, downloaded_file_name
        )
        if os.path.exists(downloaded_file_name):
            caat = await AKSHIT.client.send_file(
                event.chat_id,
...
async def _(event):
    if event.fwd_from:
        return
    AKSHITreply = await event.get_reply_message()
    if not AKSHITreply or not AKSHITreply.media or not AKSHITreply.media.document:
        return await edit_or_reply(event, "`Stupid!, This is not animated sticker.`")
    if AKSHITreply.media.document.mime_type != "application/x-tgsticker":
        return await edit_or_reply(event, "`Stupid!, This is not animated sticker.`")
    reply_to_id = event.message
    if event.reply_to_msg_id:
        reply_to_id = await event.get_reply_message()
    chat = "@tgstogifbot"
    AKSHITevent = await edit_or_reply(event, "`Converting to gif ...`")
    async with event.client.conversation(chat) as conv:
        try:
            await silently_send_message(conv, "/start")
            await event.client.send_file(chat, AKSHITreply.media)
            response = await conv.get_response()
            await event.client.send_read_acknowledge(conv.chat_id)
            if response.text.startswith("Send me an animated sticker!"):
                return await AKSHITevent.edit("`This file is not supported`")
            AKSHITresponse = response if response.media else await conv.get_response()
            await event.client.send_read_acknowledge(conv.chat_id)
            AKSHITfile = Path(
                await event.client.download_media(AKSHITresponse, "./temp/")
            )
            AKSHITgif = Path(await unzip(AKSHITfile))
            AKSHIT = await event.client.send_file(
                event.chat_id,
                AKSHITgif,
            ...
            await event.client(
                functions.messages.SaveGifRequest(
                    id=types.InputDocument(
                        id=AKSHIT.media.document.id,
                        access_hash=AKSHIT.media.document.access_hash,
                        file_reference=AKSHIT.media.document.file_reference,
                    ),
                    unsave=True,
                )
            )
            await AKSHITevent.delete()
            for files in (AKSHITgif, AKSHITfile):
                if files and os.path.exists(files):
                    os.remove(files)
        except YouBlockedUserError:
            await AKSHITevent.edit("Unblock @tgstogifbot")
            return
