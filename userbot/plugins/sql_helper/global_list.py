...
class AKSHITGloballist(BASE):
    __tablename__ = "AKSHITglobal_list"
    ...
    def __repr__(self):
        return "<AKSHIT global values '%s' for %s>" % (self.group_id, self.keywoard)

    def __eq__(self, other):
        return bool(
            isinstance(other, AKSHITGloballist)
            and self.keywoard == other.keywoard
            and self.group_id == other.group_id
        )


AKSHITGloballist.__table__.create(checkfirst=True)

AKSHITGLOBALLIST_INSERTION_LOCK = threading.RLock()
...
def add_to_list(keywoard, group_id):
    with AKSHITGLOBALLIST_INSERTION_LOCK:
        broadcast_group = AKSHITGloballist(keywoard, str(group_id))
...
def rm_from_list(keywoard, group_id):
    with AKSHITGLOBALLIST_INSERTION_LOCK:
        broadcast_group = SESSION.query(AKSHITGloballist).get((keywoard, str(group_id)))
        if broadcast_group:
...
def del_keyword_list(keywoard):
    with AKSHITGLOBALLIST_INSERTION_LOCK:
        broadcast_group = (
            SESSION.query(AKSHITGloballist.keywoard)
            .filter(AKSHITGloballist.keywoard == keywoard)
            .delete()
        )
...
def get_list_keywords():
    try:
        chats = SESSION.query(AKSHITGloballist.keywoard).distinct().all()
...
def num_list():
    try:
        return SESSION.query(AKSHITGloballist).count()
    finally:
        SESSION.close()
...
def num_list_keywords():
    try:
        return SESSION.query(func.count(distinct(AKSHITGloballist.keywoard))).scalar()
    finally:
        SESSION.close()
...
def __load_chat_lists():
    try:
        ...
        all_groups = SESSION.query(AKSHITGloballist).all()
        for x in all_groups:
