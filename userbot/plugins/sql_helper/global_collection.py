...
class AKSHIT_GlobalCollection(BASE):
    __tablename__ = "AKSHIT_globalcollection"
    ...
    def __repr__(self):
        return "<AKSHIT Global Collection lists '%s' for %s>" % (
            self.contents,
            self.keywoard,
        )
    ...
    def __eq__(self, other):
        return bool(
            isinstance(other, AKSHIT_GlobalCollection)
            and self.keywoard == other.keywoard
            and self.contents == other.contents
        )


AKSHIT_GlobalCollection.__table__.create(checkfirst=True)

AKSHIT_GLOBALCOLLECTION = threading.RLock()
...
def add_to_collectionlist(keywoard, contents):
    with AKSHIT_GLOBALCOLLECTION:
        keyword_items = AKSHIT_GlobalCollection(keywoard, tuple(contents))
...
def rm_from_collectionlist(keywoard, contents):
    with AKSHIT_GLOBALCOLLECTION:
        keyword_items = SESSION.query(AKSHIT_GlobalCollection).get(
...
def del_keyword_collectionlist(keywoard):
    with AKSHIT_GLOBALCOLLECTION:
        keyword_items = (
            SESSION.query(AKSHIT_GlobalCollection.keywoard)
            .filter(AKSHIT_GlobalCollection.keywoard == keywoard)
            .delete()
        )
...
def get_collectionlist_items():
    try:
        chats = SESSION.query(AKSHIT_GlobalCollection.keywoard).distinct().all()
...
def num_collectionlist():
    try:
        return SESSION.query(AKSHIT_GlobalCollection).count()
    finally:
        SESSION.close()
...
def num_collectionlist_items():
    try:
        return SESSION.query(
            func.count(distinct(AKSHIT_GlobalCollection.keywoard))
        ).scalar()
    finally:
        SESSION.close()
...
def __load_item_collectionlists():
    try:
        ...
        all_groups = SESSION.query(AKSHIT_GlobalCollection).all()
        for x in all_groups:
