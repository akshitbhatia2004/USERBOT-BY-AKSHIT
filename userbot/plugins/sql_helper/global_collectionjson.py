...
class AKSHIT_GlobalCollection_Json(BASE):
    __tablename__ = "AKSHIT_globalcollectionjson"
...
AKSHIT_GlobalCollection_Json.__table__.create(checkfirst=True)


def get_collection(keywoard):
    try:
        return SESSION.query(AKSHIT_GlobalCollection_Json).get(keywoard)
    finally:
        SESSION.close()
