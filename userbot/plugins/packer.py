...
@AKSHITBOT.on(admin_cmd(pattern=r"unpack", outgoing=True))
@AKSHITBOT.on(sudo_cmd(pattern=r"unpack"))
async def _(event):
...
@AKSHITBOT.on(admin_cmd(pattern="repack ?(.*)", outgoing=True))
@AKSHITBOT.on(sudo_cmd(pattern="repack ?(.*)", allow_sudo=True))
async def _(event):
